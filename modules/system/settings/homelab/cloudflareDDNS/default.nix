{
  flake.nixosModules.cloudflareDDNS =
    {
      config,
      lib,
      ...
          ...
          config =
            { pkgs, ... }:
            {
              ...
              systemd.services.${service} = {
                ...
                script = ''
                  #!/usr/bin/env bash
                  CLOUDFLARE_API_TOKEN="$(cat "/run/secrets/cloudflare-token")";
                  CLOUDFLARE_ZONE_ID="$(cat "/run/secrets/cloudflare-zoneID")";
                  CLOUDFLARE_A_PUBLIC_RECORD_IDS="$(cat "/run/secrets/cloudflare-public_recordID")";
                  CLOUDFLARE_A_PRIVATE_RECORD_IDS="$(cat "/run/secrets/cloudflare-private_recordID")";
                  #set -euo pipefail
                  echo "updating"
